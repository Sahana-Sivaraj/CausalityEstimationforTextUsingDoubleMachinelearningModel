...
class Main():
    def prepareVectorsformWord2Vec(self):
    ...
    def runProcessingTfIdfData(self,path):
    ...
    def runProcessingGenisimWordVecData(self):
        data = pd.read_csv("test.csv")
            # data = pd.read_csv("test.csv")
        temp_treatment = 'treatment'
        df = data.copy()
    ...
    def runProcessingTfWordVecDataProcess(self):
            data = pd.read_csv("test.csv")
            # print(data.head())
            temp_treatment = 'treatment'
            df = data.copy()
    ...
    def trainWithDMLNeuralNetwork(self,option):
        if option==1:
...
if __name__ == '__main__':
    main=Main()
    main.prepareVectorsformWord2Vec()
    main.runProcessingTfIdfData()
    main.runProcessingGenisimWordVecData()
    main.runProcessingTfWordVecDataProcess()
    main.trainWithDMLNeuralNetwork(option=1)
